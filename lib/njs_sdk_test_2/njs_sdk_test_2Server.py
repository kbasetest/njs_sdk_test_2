...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'njs_sdk_test_2'):
...
impl_njs_sdk_test_2 = njs_sdk_test_2(config)
...
async_run_methods = {}
async_check_methods = {}
async_run_methods['njs_sdk_test_2.run_async'] = ['njs_sdk_test_2', 'run']
async_check_methods['njs_sdk_test_2.run_check'] = ['njs_sdk_test_2', 'run']
sync_methods['njs_sdk_test_2.run'] = True
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'njs_sdk_test_2'
        ...
        self.rpc_service.add(impl_njs_sdk_test_2.run,
                             name='njs_sdk_test_2.run',
                             types=[object])
        self.method_authentication['njs_sdk_test_2.run'] = 'required'
        self.rpc_service.add(impl_njs_sdk_test_2.status,
                             name='njs_sdk_test_2.status',
                             types=[dict])
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "njs_sdk_test_2 but no authentication header was passed"
                            raise err
